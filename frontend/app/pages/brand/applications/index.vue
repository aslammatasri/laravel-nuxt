...
interface Product     { id: number; name: string; price: number; commission_rate: string; commission_type: string }
interface Creator     { id: number; name: string; email: string }
interface Application { id: number; status: string; message: string | null; pitch_message: string; created_at: string; product: Product; creator: Creator; rejection_reason?: string }
...
function formatCommission(a: Application): string {
  ...
  return a.product.commission_type === 'percentage' ? `${rate}%` : `RM ${rate}`
}
...
const columns: ColumnDef<Application, any>[] = [
    ...
    cell: info => {
      ...
      return h('div', { class: 'flex items-center gap-3' }, [
        ...
        h('div', { class: 'min-w-0' }, [
          ...
          h('span', { class: 'text-gray-400 text-xs' }, app.creator?.email),
        ]),
      ])
    },
  }),
  columnHelper.accessor(row => row.created_at, {
    id: 'applied',
    header: 'Applied',
    cell: info => h('span', { class: 'text-xs text-gray-500 whitespace-nowrap' }, formatDate(info.getValue())),
  }),
  columnHelper.accessor(row => row.product?.name, {
    ...
    cell: info => {
      const app = info.row.original
      return h('div', [
        h('p', { class: 'font-medium text-gray-700 text-sm' }, app.product?.name),
        h('p', { class: 'text-xs text-gray-400 mt-0.5' },
          `RM ${app.product?.price} · `),
        h('span', { class: 'text-xs text-green-600 font-medium' }, formatCommission(app)),
      ])
    },
  }),
  ...
  columnHelper.accessor(row => row.pitch_message, {
    ...
    cell: info => h('div', { class: 'max-w-xs' }, [
      ...
      info.row.original.status === 'rejected' && info.row.original.message
        ? h('p', { class: 'text-xs text-red-500 mt-1' }, `Reason: ${info.row.original.message}`)
        : null,
    ]),
  }),
    ...
    cell: info => {
      ...
      if (confirmingId.value === app.id) {
        return h('div', { class: 'flex items-center gap-2 whitespace-nowrap' }, [
          h('span', { class: 'text-xs text-gray-500' }, 'Confirm?'),
          h('button', {
      ...
      return h('div', { class: 'flex items-center gap-2 whitespace-nowrap' }, [
        h('button', {
          class: 'bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-colors',
          onClick: () => { confirmingId.value = app.id },
        }, 'Approve'),
        h('button', {
...
<template>
  <div>
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-500 mt-1">Review creators who want to promote your products</p>
    </div>
    ...
    <div class="flex gap-2 mb-6 flex-wrap">
      ...
      >
        ...
        <span
          ...
          :class="status === f.key ? 'bg-white/20' : 'bg-gray-100 text-gray-500'"
        >
    ...
    >
      ...
      <p class="font-medium text-gray-600">No {{ status || '' }} applications</p>
      <p class="text-sm text-gray-400 mt-1">
        {{ status === 'pending' ? 'No one has applied to your products yet.' : '' }}
        {{ status === 'approved' ? 'No approved applications yet.' : '' }}
        {{ status === 'rejected' ? 'No rejected applications.' : '' }}
        {{ !status ? 'Applications from creators will appear here.' : '' }}
      </p>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showRejectModal" class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/30" @click="showRejectModal = false" />
        <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
          ...
          <p class="text-sm text-gray-400 mb-4">Let the creator know why their application was rejected.</p>
          <textarea
