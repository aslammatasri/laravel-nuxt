...
interface VariationOption { _key: number; value: string; price: string }
interface Variation { _key: number; name: string; options: VariationOption[] }
...
function addVariation() {
  variations.value.push({ _key: nextKey(), name: '', options: [{ _key: nextKey(), value: '', price: '' }] })
}
function removeVariation(i: number) {
  variations.value.splice(i, 1)
}
function addOption(vi: number) {
  variations.value[vi]!.options.push({ _key: nextKey(), value: '', price: '' })
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const product = await $api<{ id: number }>('/brand/products', { method: 'POST', body })
    ...
    for (const v of variations.value) {
      ...
      await $api(`/brand/products/${product.id}/variations`, {
        method: 'POST',
        body: { name: v.name, options: opts.map(o => ({ value: o.value, price: parseFloat(o.price) || 0 })) },
      })
    }
      ...
      <div class="col-span-2 space-y-5">
          ...
          <div>
            ...
            <div v-if="imagePreviews.length" class="grid grid-cols-3 gap-2 mb-3">
              <div v-for="(preview, i) in imagePreviews" :key="preview" class="relative">
                ...
                <button type="button" @click="removeImageAt(i)"
              ...
              <!-- Add more button -->
              <label class="flex items-center justify-center h-28 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
            ...
            <label v-else class="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
              <span class="text-3xl mb-2">📷</span>
              <span class="text-sm font-medium text-gray-600">Click to upload images</span>
              <span class="text-xs text-gray-400 mt-1">PNG, JPG, WEBP — max 2MB each</span>
              <input type="file" accept="image/*" class="hidden" multiple @change="onImagesChange" />
            </label>
          </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          ...
          <p v-if="variations.length === 0" class="text-sm text-gray-400 text-center py-3">
            Optional — add if your product comes in sizes, flavors, pack sizes, etc.
          </p>
