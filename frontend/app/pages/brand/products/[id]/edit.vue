...
const existingImages = computed(() =>
    (product.value?.images || []).filter(
        (p) => !deleteImagePaths.value.includes(p),
    ),
);
...
const originalVariationIds = new Set(
    (product.value?.variations ?? []).map((v) => v.id!),
);
...
function addVariation() {
    variations.value.push({
        _key: nextKey(),
        name: "",
        options: [{ _key: nextKey(), value: "", price: 0 }],
    });
}
...
async function handleSubmit() {
    ...
    try {
        ...
        deleteImagePaths.value.forEach((p) =>
            body.append("delete_images[]", p),
        );
        ...
        const currentIds = new Set(
            variations.value.filter((v) => v.id).map((v) => v.id!),
        );
        for (const id of originalVariationIds) {
            if (!currentIds.has(id)) {
                await $api(`/brand/products/${productId}/variations/${id}`, {
                    method: "DELETE",
                });
            }
        }
        ...
        for (const v of variations.value) {
            ...
            if (v.id) {
                await $api(`/brand/products/${productId}/variations/${v.id}`, {
                    method: "PUT",
                    body: varBody,
                });
            } else {
                await $api(`/brand/products/${productId}/variations`, {
                    method: "POST",
                    body: varBody,
                });
            }
            ...
            <div class="col-span-2 space-y-5">
                ...
                >
                    ...
                    <div>
                        ...
                        <!-- Add more button -->
                        <label
                            class="flex items-center justify-center w-full h-28 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors"
                        >
