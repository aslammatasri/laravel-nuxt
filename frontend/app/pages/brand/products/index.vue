...
interface Product { id: number; name: string; image: string | null; images: string[]; status: string; commission_rate: string; commission_type: string; category: string; description: string; price: number }
...
<template>
  <div>
    ...
    >
      ...
      <p class="text-sm mt-1">Add your first product to start finding creators</p>
      <NuxtLink
        to="/brand/products/create"
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      >
        ...
        <div class="h-40 overflow-hidden bg-gray-100">
          <img v-if="product.images?.[0]" :src="`${config.public.storageBase}/${product.images[0]}`" :alt="product.name" class="w-full h-full object-cover" />
        ...
        <div class="p-5">
          <div class="flex items-start justify-between mb-2">
            ...
            <span
              ...
              :class="{
                ...
                'bg-gray-100 text-gray-500':   product.status !== 'active',
              }"
            >
              {{ product.status }}
            </span>
          ...
          <div class="flex items-center justify-between text-xs text-gray-500 mb-4">
            ...
            <span class="font-medium text-green-600">
              {{ product.commission_rate }}{{ product.commission_type === 'percentage' ? '%' : ' RM' }} commission
            </span>
          </div>
